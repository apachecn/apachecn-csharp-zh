...
在上一章中，您了解了树木。然而，您知道这样的数据结构也属于图吗？但什么是图，以及如何在应用中使用它？您可以在本章中找到这些问题和许多其他问题的答案！

首先，将介绍有关图的基本信息，包括节点和边的说明。此外，您将看到有向边和无向边之间的差异，以及加权边和未加权边之间的差异。由于图是实践中常用的数据结构，您还将看到一些应用，例如在社交媒体中存储朋友的数据或在城市中查找道路。然后，将讨论图表示的主题，即使用邻接列表和矩阵。
...
下一部分将介绍最小生成树的主题，以及创建它们的两个算法，即 Kruskal 和 Prim。这些算法将以文本描述、基于 C 的代码片段以及易于理解的插图的形式呈现。此外，还将提供示例真实应用。

另一个与图相关的有趣问题是节点的着色，这将在本章的下一部分中予以考虑。最后，将使用 Dijkstra 算法分析在图中寻找最短路径的主题。当然，还将展示示例真实世界应用以及基于 C# 的实现。
...
在简短的介绍之后，您将了解一些关于图的基本信息，特别是关于节点和各种边的信息。然而，为什么图的主题如此重要，为什么它占据了本书的整个章节？您能在应用中使用此数据结构吗？答案显而易见：是的！这些图通常用于解决各种算法问题，并有许多实际应用。下图显示了两个示例。
...
您已经了解了一些关于图的基本信息，包括节点、边和两种表示方法，即使用邻接列表和矩阵。但是，如何在应用中使用这种数据结构？在本节中，您将学习如何使用 C 语言实现图。为了使您更容易理解所呈现的内容，提供了两个示例。
...
这就是全部！您刚刚完成了与 MST 的实际应用相关的示例。您是否准备好继续学习另一个与图相关的主题，即着色？
...
在这个简短的介绍之后，您就可以开始实际的应用了，即为 voivodeship 贴图着色，下面将介绍这个应用。
...
在这篇简短的介绍之后，结合简单的示例，让我们继续讨论与游戏开发相关的更高级、更有趣的应用。走吧！
...
您刚刚完成了与开发应用时可用的最重要数据结构之一相关的一章，即图。如您所知，图是由节点和边组成的数据结构。每条边连接两个节点。此外，图中有各种各样的边，例如无向边和有向边，以及未加权边和加权边。所有这些都已详细描述和解释，并附有图表和代码示例。本文还解释了两种图表示方法，即使用邻接表和邻接矩阵。当然，您还学习了如何使用 C 语言实现图。

在讨论图时，展示一些实际应用也很重要，特别是由于这种数据结构的普遍使用。例如，本章描述了社交媒体中朋友的结构，或在城市中搜索最短路径的问题。
