...
第二章,*设置 ML.NET 环境,详细谈论开始使用 ML.NET,持续的机器学习的概述和 ML.NET 如何帮助发展中国家在新的和现有的应用并运行模型。 您将确保您的开发环境已经设置好，本章以控制台应用中的一个简单的预训练模型结束，以证明您已经准备好进行训练。*
...
第四章、*分类训练器模型*讨论了在 ML.NET 中使用分类训练器模型以及分类模型可以帮助解决哪些问题。 在本章中，我们将创建两个应用来演示 ML.NET 中对分类训练器的支持。 第一个是通过 ML.NET 提供的 FastTree 训练器，根据几个属性和比较价格来预测一辆车是否有价值。 第二个应用使用 ML.NET 中的 SDCA 训练器获取电子邮件数据(主题、正文、发件人)，将电子邮件分类为订单、垃圾邮件或好友。 通过这些应用，您还将了解如何评估分类模型。

第五章、*聚类模型*讨论了在 ML.NET 中使用 k-means 聚类训练器以及聚类模型可以帮助解决哪些问题。 在本章中，我们将使用 ML.NET 提供的 k-means 集群训练器来创建一个示例应用，该应用将文件分为可执行文件、文档或脚本。 此外，您还将学习如何在 ML.NET 中评估聚类模型。
...
[第九章](09.html)，*在 ASP 中使用 ML.NET。 asp.net Core*，介绍了一个使用 ASP 的真实应用。 NET 与前端上传一个文件，以确定它是否是恶意的。 本章的重点是使用二进制分类器以及如何将其集成到 ASP.NET 应用。
...
第 11 章、*训练和建立生产模型*涵盖了所有考虑因素的大规模训练模型，以及使用 DMTP 项目对生产模型的适当训练。 得到的教训包括获得适当的训练集(多样性是关键)，适当的特征，以及您的模型的真实评估。 本章的重点是训练生产准备模型的技巧、技巧和最佳实践。
