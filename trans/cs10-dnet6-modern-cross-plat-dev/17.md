...
*   **.NET 毛伊岛 Blazor 应用**又名**Blazor Hybrid**，运行在的.NET 进程中，使用本地互操作通道将其web UI 呈现给 web 视图控件，并托管在.NET 毛伊岛应用中。它在概念上类似于使用 Node.js 的电子应用。我们将在在线章节*第 19 章*中看到这种托管模式，*使用.NET 毛伊岛*构建移动和桌面应用。
...
Blazor WebAssembly可选支持**渐进式 Web 应用**（**PWAs**），这意味着网站访问者可以使用浏览器菜单将应用添加到桌面，并离线运行应用。
