...
*   创建一个 ASP.NET 核心 MVC 应用并在一个容器中运行它
...
接下来，我们将了解 Docker 如何集成到 Visual Studio 2017 中，以及您可以做些什么来支持您的 ASP.NET 核心应用。我们还将了解如何添加 Docker 支持(或 Docker 化)现有的 ASP.NET 核心应用。

Docker 有一个健康的开发人员社区，它也有很多可用的帮助文档。花点时间浏览一下，研究一下你可能遇到的任何问题。

# 在 Docker 中运行 Visual Studio 2017 中的 ASP.NET 核心应用

那么，这一切会给我们留下什么呢？我们已经了解了如何在 Windows 10 上设置 Docker，以及如何解决围绕该设置的一些问题。现在让我们看看如何创建一个 ASP.NET 核心应用，并向新应用添加 Docker 支持。

1.  在 Visual Studio 2017 中创建新的 ASP.NET 核心网络应用，然后单击确定:

![](img/56d75944-f66e-4307-938a-991eb3e24ce1.png)

2.  在下一个屏幕上，选择网络应用(模型-视图-控制器)或您喜欢的任何类型，同时确保从下拉列表中选择了 ASP.NET 核心 2.0。然后选中启用 Docker 支持复选框。这将启用操作系统下拉列表。在此选择窗口，然后单击确定按钮:
...
创建 ASP.NET 核心应用后，您将在解决方案资源管理器中看到以下项目设置:
...
4.  点击 Docker 按钮来调试你的 ASP.NET 核心应用，你会注意到输出窗口中出现了一些东西。特别感兴趣的是结尾的 IP 地址。在我的例子中，它的内容是启动 http://172.24.12.112(您的会有所不同):

![](img/cb41f852-e3b2-49a5-a426-04afafc9b495.png)

当浏览器启动时，您将看到 ASP.NET 核心应用正在先前在输出窗口中列出的 IP 地址上运行。您的 ASP.NET 核心应用现在正在 Windows Docker 容器中运行:

![](img/c725afa8-0fe4-482b-a476-24d8c642e639.png)

这很棒，也很容易上手。但是，您需要做什么来归档现有的 ASP.NET 核心应用呢？事实证明，这并没有你想象的那么难。

# 向现有的 ASP.NET 核心应用添加 Docker 支持

假设您有一个没有 Docker 支持的 ASP.NET 核心应用。要将 Docker 支持添加到这个现有的应用中，只需从上下文菜单中添加它:

![](img/64a64c14-60a7-4988-b714-d5e1bf29d3eb.png)

要向现有的 ASP.NET 核心应用添加 Docker 支持，您需要执行以下操作:
...
创建内置 Docker 支持的 ASP.NET 核心应用非常容易，向现有的 ASP.NET 核心应用添加 Docker 支持更容易。
...
11.  为了测试这一点，请在 Visual Studio 2017 中打开您的 ASP.NET 核心应用并进行一些更改。然后将这些更改提交到您的 GitHub 存储库中。然后点击 Docker Hub 中的构建细节链接。您将看到构建已经排队，并将在几分钟后完成。要查看构建结果，请稍后刷新此页面:
...
23.  打开浏览器，输入 IP 地址和端口号为`10.0.75.1:5000`，点击*进入*。您的 ASP.NET 核心应用将在您的浏览器窗口中弹出，光彩夺目:
