...
当谈到在您的应用中使用 MongoDB 时，人们想知道将这一功能添加到一个新的 ASP.NETCore MVC 应用中会有多容易。这个过程真的很简单。首先，创建一个新的 ASP.NETCore网络应用并命名为`BugTracker`:

1.  在新 ASP.NETCore网络应用-错误跟踪器屏幕上，确保您已经从下拉列表中选择了 ASP.NETCore 2.0。
2.  选择网络应用(模型-视图-控制器)。
