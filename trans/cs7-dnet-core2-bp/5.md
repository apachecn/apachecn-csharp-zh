...
为了展示SignalR的能力，我们将构建一个简单的 ASP.NETCoreSignalR聊天应用。这将包括使用 NuGet 和**节点包管理器** ( **npm** )向项目添加所需的包文件。
...
1.  使用 Visual Studio 2017，我们将创建一个 ASP.NETCore网络应用。你可以随意调用应用，但我调用了我的`Chapter5`:
...
在撰写本文时，在 NuGet 包管理器中浏览时找不到 ASP.NETCore信号器的包，因此我们将使用包管理器控制台添加所需的包。
...
当然，我们这里有一个小的错误窗口:如果用户没有输入要保存的文件的有效路径，代码将抛出一个异常。我会让你自己的创造力来解决这个问题。我来这里只是为了SignalR的功能。
...
1.  以下是我们通过 NuGet 添加的 signor Asp.NETCore库:
