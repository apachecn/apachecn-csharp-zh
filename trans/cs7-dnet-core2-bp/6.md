...
使用 Visual Studio 2017，创建一个 ASP.NET 核心网络应用。请注意，我们在这个项目中采用了代码优先的方法:
...
控制器是构建 ASP.NET 核心 MVC 应用的基本构件。控制器内部的方法被称为动作。因此，我们可以说控制器定义了一组动作。操作处理请求，这些请求通过路由映射到特定的操作。
