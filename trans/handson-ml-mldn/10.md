...
现在我们已经建立了生产级。net 核心如何创建一个控制台应用,在本章中,我们将深入地了解创建一个功能齐全的 Windows 10 应用与**环球 Windows 平台**(**UWP)框架。 本应用将利用 ML.NET 二进制分类模型对网页内容进行分类，以确定内容是良性还是恶性的。 此外，我们将探索将代码分解为基于组件的体系结构，使用. net 标准库在桌面应用和控制台应用之间共享，控制台应用将训练我们的模型。 在本章结束时，您应该已经掌握了如何使用 ML.NET 设计和编码产品级 UWP 桌面应用。**
