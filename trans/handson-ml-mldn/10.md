...
现在我们已经建立了生产级。NETCore如何创建一个控制台应用,在本章中,我们将深入地了解创建一个功能齐全的 Windows 10 应用与**环球 Windows 平台**(**UWP)框架。 本应用将利用 ML.NET 二进制分类模型对网页内容进行分类，以确定内容是良性还是恶性的。 此外，我们将探索将代码分解为基于组件的体系结构，使用. net 标准库在桌面应用和控制台应用之间共享，控制台应用将训练我们的模型。 在本章结束时，您应该已经掌握了如何使用 ML.NET 设计和编码产品级 UWP 桌面应用。**
...
到目前为止，在我们的所有示例中使用的`Constants`类是包含在库、培训器和 UWP 应用中使用的常量值的公共类。 在本章中，添加了`MODEL_NAME`和`MALICIOUS_THRESHOLD`属性来保存我们的模型名称，以及一个任意的阈值，用来分别决定我们的预测是否属于恶意预测。 如果你发现你的模型太敏感，试着调整这个阈值，像这样:
...
现在我们已经回顾了共享库和桌面应用，让我们进入培训器应用。 在第 8 章的示例中执行了主要的架构更改，通过设计，培训器应用只进行了最小的更改，以处理本章示例中使用的特定类对象。
...
要开始运行培训器应用，我们需要首先运行`chapter10.trainer`应用，在训练模型之前执行特征提取。 为了运行培训师应用，该过程几乎与第 9 章的示例应用相同，只是在培训时添加了传递测试数据集文件名路径:

1.  运行培训师应用，传递到培训和测试 URL 列表 csv 的路径，进行特征提取，如下所示:
