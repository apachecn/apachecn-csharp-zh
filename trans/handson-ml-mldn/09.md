...
*   Blazor (ASP。 NET 核心托管的)客户端应用。 NET Core(这是我们将在下一节中回顾的项目类型)。
...
现在，我们已经回顾了共享库和 web 应用，让我们进入培训器应用。
...
1.  我们将运行培训师应用，将路径传递到培训和测试文件夹，进行特征提取，如下所示:
...
在本章的学习过程中，我们已经讨论了什么是可用于生产的 ASP。 NET Core Blazor web 应用架构，使用前面章节的工作作为基础。 我们还利用 ML.NET 中的 FastTree 二进制分类器创建了一个全新的文件分类 web 应用。 最后，我们还讨论了进一步增强 ASP 的一些方法。 NET 核心应用(以及一般的生产应用)。
