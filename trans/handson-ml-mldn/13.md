...
既然我们已经完成了我们的深入探究用 TensorFlow**Windows Presentation Foundation (WPF**)应用和 ML.NET,现在,是时候深入使用**打开神经网络交换**(**ONNX**),ML.NET。 特别地，在最后一章中，我们将回顾什么是 ONNX，并使用预先训练的 ONNX 模型**YOLO**创建一个新的示例应用。 这个应用将建立在前一章的基础上，并显示模型检测到的对象的边界框。 此外，我们还将在本章结束时提出关于改进示例的建议，使其成为生产级应用或集成到生产应用中。****
...
如前所述，我们将创建的应用是一个使用预训练的 ONNX 模型的对象检测应用。 使用我们开发的应用在 12 章,*用 TensorFlow ML.NET*作为起点,我们将在支持添加图像的边框上覆盖的模型分类对象很清楚。 这对一般公众的有用之处在于图像对象检测提供的各种应用。 想象一下，你正在为警察或情报机构做一个项目，他们有图像或视频，想要检测武器。 正如我们将要展示的，在 ML.NET 中使用 YOLO 模型将使这个过程变得非常简单。
...
对于这个应用，正如在前一节中提到的，我们是在[第 12 章](12.html)、*中使用 ML.NET*中的 TensorFlow 完成的工作之上构建的。 虽然**用户界面**(**UI**)没有发生太多变化，但是运行 ONNX 模型的底层代码发生了变化。 对于每一个被更改的文件(如前几章所述)，我们将回顾所做的更改以及这些更改背后的原因。
...
我们应用的**可扩展应用标记语言**(**XAML**)视图已经简化为按钮和图像控件，如下代码块所示:
...
在对`Classify`方法进行了相应的更改之后，本章示例所需的代码更改就完成了。 现在，让我们运行应用!

# 运行应用

要运行应用，该过程与第 12 章、*中使用 TensorFlow 与 ML.NET*中的示例应用相同。 要在 Visual Studio 中运行应用，只需单击工具栏中的*播放*图标，如下截图所示:

![](img/5fe7e40a-4da8-4dc1-8700-6fd1782f74bd.png)

在启动应用后，就像在[第 12 章](12.html)，*使用 TensorFlow 与 ML.NET*，选择一个映像文件，模型就会运行。 例如，我选择了我在德国度假时拍摄的一张图片(注意汽车的边框)，如下截图所示:
...
现在我们已经完成了深入的研究，还有几个额外的元素可以进一步增强应用。 接下来的部分将讨论一些想法。

# 日志记录

如前所述，日志的重要性在桌面应用中再怎么强调也不为过。 随着应用复杂性的增加，强烈推荐使用 NLog([https://nlog-project.org/](https://nlog-project.org/))或类似的开源项目进行日志记录。 这将允许您在不同级别上记录文件、控制台或第三方日志解决方案(如 Loggly)。 例如，如果将此应用部署到客户，将错误级别至少划分为 Debug、Warning 和 error 将有助于远程调试问题。
...
此外，该示例的另一个改进方面是在应用中使用完整的 YOLO 模型。 如前所述，示例应用中使用的 Tiny YOLO 模型只提供了 20 个标签。 在生产应用或希望在其上进行构建的应用中，使用更大、更复杂的模型将是一个不错的选择。
