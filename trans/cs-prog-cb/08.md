...
9.  返回到控制台应用，并将以下`using`语句添加到您的控制台应用`Program.cs`文件中:
...
4.  在控制台应用中，将相关的`using`语句添加到`Program.cs`类中，以将静态类纳入范围:
...
4.  在控制台应用中，将相关的`using`语句添加到`Program.cs`类中，将静态类纳入范围:
...
14.  在控制台应用中，将相关的`using`语句添加到`Program.cs`类中，将`Chapter8`类纳入范围:
...
7.  在控制台应用中，将相关的`using`语句添加到`Program.cs`类中，将静态类纳入范围:
...
10.  然后，修改您的调用代码，并为`MethodOne()`传递一个有效值，但保持调用不变。再次运行您的控制台应用:
...
6.  在控制台应用中，向`Program.cs`类添加相关的`using`语句，将`Chapter8`类纳入范围:
