...
为了为开发者创造一个云优化的、跨平台的.NET Framework 实现，解决.NET Framework 的局限性，微软开始使用.NET Framework 开发一个.NET Core 平台。 随着.NET 1.0 版本的引入核心 2016 年,在.NET 平台上的应用开发中扮演了一个新的维度,随着.NET 开发人员现在可以构建应用工作在 Windows、Linux, macOS,和云,嵌入式,轻松和物联网设备.NETCore与.NET 框架兼容,Xamarin 的, 以及通过.NET 标准实现的 Mono。
...
显然,当您正在开发的应用是运行在多个平台上,应该使用. NETCore.NETCore是跨平台的使它适合开发服务和 web 应用,可以运行在 Windows**,**Linux**,**macOS【5】。 同时,通过引入**【T7 Visual Studio 代码】通过微软成为一个编辑器完全支持.NETCore提供智能感知和调试功能,以及其他 IDE 特性,通常只能在 Visual Studio IDE【显示】**。****
...
当应用将使用一个技术不可用.NETCore,例如使用 WPF 或 Windows 窗体的表示层,WCF 服务器实现,甚至第三方库,目前没有.NETCore版本,建议你最好使用.NET 框架开发应用。
...
ASP 需要`Startup`类。 NETCore应用来管理应用的请求管道、配置服务和依赖注入。
...
开发.NETCore应用变得更加容易，这不仅是因为平台的流畅性和健壮性，还因为引入了**Visual Studio Code**，这是一个可以在 Windows、Linux 和 macOS 上运行的跨平台编辑器。 在.NET Core 上创建应用之前，你不需要在你的系统上安装 Visual Studio IDE。
...
3.  打开先前创建的控制台应用的文件夹; ,点击文件菜单,选择打开文件夹或按*Ctrl*+*K*,*Ctrl + o*这将打开文件管理器; 浏览到文件夹的路径并单击打开。 这将在 Visual Studio Code 中加载项目的内容。 在后台，Visual Studio Code 将尝试下载所需的依赖 Linux 平台，Omnisharp 为 Linux 和.NETCore调试器:**
...
ASP。 NET Core 使用 Kestrel 运行，Kestrel 是 asp.net 中包含的一个 web 服务器。 NETCore项目模板。 Kestrel 是一个基于**libuv**的进程内跨平台 HTTP 服务器实现，libuv 是一个跨平台异步 I/O 库。 NET Core 应用更简单。 它侦听 HTTP 请求，然后将请求细节和特性打包到一个`HttpContext`对象中。 Kestrel 可以作为一个独立的 web 服务器使用，也可以与 IIS 或 Apache web 服务器一起使用，在这些服务器上，其他 web 服务器接收到的请求会被转发到 Kestrel，这是一个被称为反向代理的概念。
