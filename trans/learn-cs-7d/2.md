...
*   它由一个健壮耐用的应用程序构建，例如*垃圾收集*(在第一天讨论)
...
在我们开始用 C#编写程序之前，让我们先回到第一天，在那里我们讨论了各种有助于使用 C#语言编写程序/应用程序的 ide 和编辑器。重温第一天，了解各种编辑器和 IDEs，并检查为什么我们应该选择一个。我们将在本书的所有示例中使用 Visual Studio 2017 更新 3。
...
要从一个简单的 C#程序开始(我们将创建一个控制台应用程序)，请执行以下步骤:

1.  启动您的 Visual Studio。
2.  转到文件|新建|项目(或*ctrl*+*Shift*+*N*)。
3.  在 Visual C#节点下，选择。NET 核心，然后选择控制台应用程序。
...
在继续之前，让我们分析一下我们在 Visual Studio 上的控制台应用程序的下图:
...
这是我们定义程序/应用程序中将要使用的名称空间的地方。通常，这是使用语句来调用的，该语句包括外部、内部或任何其他名称空间的使用。
...
入口是一种让 CLR 知道 DLL 中函数的*什么*和*在哪里*的方式。例如，每当我们运行控制台应用程序时，它都会告诉 CLR`Main`是入口点，一切都围绕在这里。详见[https://docs . Microsoft . com/en-us/dotnet/framework/interop/指定入口点](https://docs.microsoft.com/en-us/dotnet/framework/interop/specifying-an-entry-point)和[https://docs . Microsoft . com/en-us/dotnet/framework/interop/指定入口点](https://docs.microsoft.com/en-us/dotnet/framework/interop/specifying-an-entry-point)。

# 5(第 02 天)

这是我们的控制台应用程序的解决方案的名称。

一个解决方案可以包含许多库、应用程序、项目等等。例如，我们的解决方案第 02 天将包含另一个名为第 03 天或第 04 天的项目。我们的控制台应用程序的 Visual Studio 解决方案文件名是`Day02.sln`。
...
依赖项是我们的应用程序所依赖的程序集或 dll，或者是我们的应用程序正在使用引用的程序集的功能的地方。例如，我们的控制台应用程序需要。NET Core 2.0 SDK，所以它包含它作为依赖项。参考以下截图:
...
# 使用 Visual Studio 深入研究应用程序

在前一节中，您了解了我们的控制台应用程序可以包含的各种内容。在这一节中，让我们深入了解如何使用 Visual Studio 来获得更多信息。

要开始，请转到项目属性。从解决方案资源管理器(右键单击项目并单击属性)或从菜单(项目| Day02 属性)执行此操作；您将看到项目属性窗口，如下图所示:

![](img/00018.jpeg)

在应用程序选项卡上，我们可以设置程序集名称、默认命名空间、目标框架和输出类型(输出类型为`Console Application`、`Windows Application`、`Class Library`)。
...
我们已经浏览了控制台应用程序，并讨论了典型的控制台应用程序包含什么，以及如何使用 Visual Studio 设置各种东西。现在让我们讨论我们的代码，它是在前面的章节*中编写的，理解一个典型的 C#程序*。
...
还有一些方法在使用控制台应用程序时很有帮助。关于这些方法的更多细节，请参考[https://docs.microsoft.com/en-us/dotnet/api/system.console?view=netcore-2.0](https://docs.microsoft.com/en-us/dotnet/api/system.console?view=netcore-2.0) 。
...
*   **public** :这是一个访问修饰符，通过应用程序设置可用性，将在第四天讨论。
...
|  | &#124;&#124; | 这是一个合乎逻辑的`OR`运算符。如果任何操作数为真，表达式的计算结果为真，例如`var result = num1 &#124;&#124; num2;` |
|  | ！ | 这被称为逻辑`NOT`运算符。它反转评估结果，例如`var result = !(num1 && num2);` |
| 按位运算符 | &#124; | 这是一个按位`OR`运算符，用于位。如果任一位为 1，结果将为 1，例如`var result = num1 &#124; num2;` |
