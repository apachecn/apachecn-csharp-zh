...
![](assets/5237ca31-885c-432b-9361-5d3ae572ecec.png)
...
![](assets/481b8095-a846-48c1-95e7-9ec15d692b72.png)

测试用例执行的输出可以在 test Explorer 窗口中看到:

![](assets/0c2e553e-e26e-4527-9f74-14522d4b2cb1.png)
...
![](assets/83e88031-cd3f-4584-be32-5e41af5e4369.png)
...
*   NSubstitute(在。net 核心中不支持)
*   Rhino Mocks(在。net 核心中不支持)
*   Moq(在。net 核心中支持)
*   NMock3(在。net 核心中不支持)
...
前面的`Controller`类还公开了一个名为`DisplayData()`的异步方法，该方法从服务中获取数据并将其写入控制台。 当我们试图为上述方法编写单元测试用例时，我们将遇到的第一个问题是，在没有任何具体实现的情况下，我们无法创建服务实例。 即使我们确实有一个具体的实现，我们也应该避免调用实际的服务方法，因为这更适合于集成测试用例而不是单元测试用例。 嘲笑在这里拯救了我们。
...
![](assets/1d4af856-f5cb-4377-858c-7b492f84903d.png)
