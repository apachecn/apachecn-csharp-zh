...
如你所见，我们将最小工作线程和 IOCP 线程增加到 25 个。 随着更多的请求进入，将创建更多的线程。 这里需要注意的重要一点是，由于每个请求都被分配了一个唯一的线程，我们应该避免编写阻塞代码。 使用阻塞代码，将不会有空闲线程。 一旦线程池耗尽，请求将开始排队。 IIS 每个应用程序池最多只能队列 1,000 个请求。 我们可以通过更改`machine.config`文件中的`requestQueueLimit`应用程序设置来修改这一点。
...
![](assets/804457ce-c889-4d44-a876-1cfc35f55cc6.png)
...
![](assets/7775bc70-bd55-45fd-adc5-a2549b9067b5.png)

下图显示了两个带有专用线程池的微服务:

![](assets/4e7cde3f-7af7-493f-97a4-4f1028f7eb21.png)
...
![](assets/6af4da68-75e4-42f2-b4cb-2ed6c9f106dc.png)

2.  给项目一个名字和你想要创建它的位置:

![](assets/2b29b76f-9f1c-42ad-836c-cb80ee916aee.png)

3.  选择项目的类型，在我们的例子中是 API，然后单击 Create:

![](assets/5bbfee2e-3a88-4f7a-9a0c-9a02bd5c088d.png)

4.  现在，在我们的项目中创建一个名为`Files`的新文件夹，并添加一个名为`data.txt`的文件，该文件包含以下内容:

![](assets/3baa8323-ae99-45b4-9e78-c32b367b58aa.png)
...
![](assets/a4c7c02d-65c0-4032-95d3-5e245be1555f.png)
