...
在本章中，我们将为婚介应用创建基本功能。然而，由于隐私问题，我们不会对人们进行评级。相反，我们将从互联网上随机下载图片。这个项目是为任何想要介绍如何编写可重用控件的人准备的。我们还将研究如何使用动画使我们的应用使用起来感觉更好。此应用将不是**模型视图视图模型**（**MVVM**应用，因为我们希望将控件的创建和使用与 MVVM 的轻微开销隔离开来。
...
在本项目中，我们将了解更多有关创建可重用控件的信息，这些控件可以添加到**可扩展应用标记语言**（**XAML**页面中。为了简单起见，我们将不使用 MVVM，而是使用没有任何数据绑定的裸机 Xamarin.Forms。我们的目标是创建一个应用，让用户可以像大多数流行的婚介应用一样，左右滑动图像。
...
如果没有屏幕两侧的特殊放置区，婚介应用就什么都不是了。我们想在这里做几件事，如下所示：
