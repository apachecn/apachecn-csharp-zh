...
CoreCLR 为提供公共语言运行时环境.NETCore应用，并管理整个应用生命周期的执行。它在程序运行时执行各种操作。内存分配、垃圾收集、异常处理、类型安全、线程管理和安全性等操作都是 CoreCLR 的一部分。
...
CLR 被实现为一组随应用一起加载的进程内库，并在应用进程的上下文中运行。在下图中，我们有两个.NETCore应用运行，命名为 App1.exe 和 App2.exe*。*每个黑框代表应用进程地址空间，App1.exe 和 App2.exe 的应用在此并行运行各自的 CLR 版本:

![](img/00027.gif)

包装时.NET Core 应用，我们可以将其发布为**框架依赖部署** ( **FDDs** )或**自包含部署** s ( **SCDs** )。在 FDDs 中，发布的包不包含.NETCore运行时，并期望.NET Core 存在于目标/宿主系统上。对于 SCDs，所有组件，例如.NET Core 运行时和.NETCore库包含在发布的包中。不需要在目标系统上安装 NET Core。
