...
作为一个新的应用平台，Windows8具有向后兼容性，允许运行普通的 Windows 应用。这导致了一种情况，即存在两大类应用：Windows 应用商店应用，其中新程序通过 Windows 应用商店分发，以及自上一版本 Windows 以来未更改的常见经典应用。
...
在这里，我们使用了一种相当复杂的方式从一个常见的.NET 控制台应用中使用WinRT API。不幸的是，并非所有可用的 API 都能在这种情况下工作，但使用运动传感器、GPS 定位服务等仍然是有用的。
