...
Xamarin 是一个基于**Mono**的 CLI 实现，它是一个跨平台的、开源的.NET 框架。 总的来说，Mono api遵循的是.NET Framework 的发展，而不是.NET Core。 该框架旨在编写可以在 iOS、Android、macOS 和 Windows 设备上运行的移动应用。
...
程序集是用于部署、版本控制和安全性的基本单元。 程序集有两种形式，要么是**可执行文件**(**.exe**)，要么是**动态链接库**(**.dll**)。 集合是类型、资源和元信息的集合，它们构成一个功能的逻辑单元。 程序集只有在需要时才被加载到内存中。 对于.NET 框架应用，程序集可以位于应用专用文件夹中，也可以在全局程序集缓存中共享，前提是它们是强命名的。 对于. net Core应用，后一种解决方案是不可用的。
...
GAC 不用于.NET Core 程序集。 这些程序集可以在任何平台上运行，而不仅仅是Windows 上。 在. net Core 2.0 之前，部署的唯一选项是应用文件夹。 但是，从 2.0 版本开始，可以根据目标环境中存在的一组已知包打包和部署应用。 这使得部署速度更快，磁盘空间要求更低。 通常，这个存储在 macOS 和 Linux 上的**/usr/local/share/dotnet/store**中，在 Windows 上的**C:/Program Files/dotnet/store**中。
