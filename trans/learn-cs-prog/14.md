 ...
 *您可能需要使用*进一步阅读*部分中提供的链接来研究更高级的调试技术。 它们包括转储的创建，转储是一个二进制文件，包含崩溃时应用进程的内存。 稍后可以使用调试工具调查转储。 另一个强大的和非常先进的工具是**儿子**【5】【T7 罢工】【显示】(**SOS)插件可用从 WinDbg 调试器(Windows)或**【病人】dotnet-dump 分析。net 核心工具。****
