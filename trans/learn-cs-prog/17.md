...
![Figure 17.1 - Visual Studio 2019 unit-testing project templates ](image/Figure_17.1_B12346.jpg)
...
![Figure 17.2 - The Test Explorer in Visual Studio showing the result of executing the empty unit test created from the selected template. ](image/Figure_17.2_B12346.jpg)

图 17.2 - Visual Studio 中的测试资源管理器显示了执行从所选模板创建的空单元测试的结果

你可以手动添加更多的单元测试类，也可以使用 Visual Studio 中可用的测试模板，如下图所示:

![Figure 17.3 - The Add New Item dialog in Visual Studio with a selection of unit test items. ](image/Figure_17.3_B12346.jpg)
...
![Figure 17.4 - The Test Explorer showing the successful execution of the test methods written earlier. ](image/Figure_17.4_B12346.jpg)
...
![Figure 17.5 - The Test Explorer showing the execution of the test methods with the TestProperties() method failed. ](image/Figure_17.5_B12346.jpg)

图 17.5 - Test Explorer 显示 TestProperties()方法失败后测试方法的执行

失败的原因在**测试详细信息摘要**窗格中详细说明，如下面的屏幕截图所示。 当您单击失败的测试时，将显示此窗格:

![Figure 17.6 - The Test Detail Summary pane of the Test Explorer showing details about the failed test. ](image/Figure_17.6_B12346.jpg)
...
![](image/Chapter_17_Table_1_01.jpg)
...
![Figure 17.7 - The Test Explorer window showing the successful execution of all the unit tests, including the ones written for the Rectangle extension methods.](image/Figure_17.7_B12346.jpg)
...
![Figure 17.8 - The Code Coverage Results pane in Visual Studio showing the code coverage of our unit tests.](image/Figure_17.8_B12346.jpg)
...
![](image/Chapter_17_Table_2_01.jpg)
...
然后我们可以使用上下文访问数据源信息。 在下面的示例中，我们重写了测试方法，以便从一个名为**TestData.csv**的 CSV 文件中获取数据，该文件位于与测试应用程序相同的文件夹中。 该文件的内容如下:
...
如果不希望在源代码中指定数据源信息(例如连接字符串)，则可以使用应用程序配置文件来提供这些信息。 为此，您必须添加一个自定义部分，然后定义连接字符串(包含名称、字符串和提供者名称)和数据源(包含名称、连接字符串名称、表名称和数据访问方法)。 对于我们在前面的示例中使用的CSV 文件，**App.config**文件如下所示:
