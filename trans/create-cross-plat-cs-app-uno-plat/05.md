...
在本章中，我们将构建 UnoBookRail**ResourcePlanner**应用，它将在UnoBookRail 内部使用。 UnoBookRail 的员工将能够使用这个应用来管理 UnoBookRail 内的任何资源，比如火车和车站。 在本章中，我们将开发该应用的问题管理部分。虽然该应用的真实版本会有更多的功能，但在本章中，我们将只开发以下功能:
...
由于 UnoBookRail的员工会使用这个应用来管理现有的问题，所以查看所有的问题，从而轻松地了解自己当前的状态，对他们来说很重要。 虽然没有内置的 UWP 和 Uno 平台控件来方便实现，但幸运的是，Windows 社区工具包包含了针对这种情况的正确控件:**DataGrid**。
...
5.  Lastly, replace the creation of **SavePDFClickedCommand** with the following code:
    ...
    在本章中，我们将文件写入一个临时文件夹，并使用默认的 PDF 查看器打开它。 根据您的应用和使用case，**FileSavePicker**和其他文件选择器可能非常适合。 您可以了解更多关于**FileSavePicker**和其他可用的文件选择器的信息:[https://platform.uno/docs/articles/features/windows-storage-pickers.html](https://platform.uno/docs/articles/features/windows-storage-pickers.html)。
...
虽然Uno 平台的关键特性是运行可以在所有平台上运行的代码，但它也允许开发人员编写特定于平台的自定义控件。 您可以使用它来利用特定于平台的控件。 在本例中，我们将使用它来创建一个 HTML**a-tag**来为我们的应用的 WASM 版本提供一个下载链接。我们将使用**Uno.UI.Runtime.WebAssembly.HtmlElement**属性:
