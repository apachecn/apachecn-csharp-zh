...
![Figure 4.1 – Before and after screenshots showing content allowing for the status bar and camera notch ](image/Author_Figure_4.01_B17132.jpg)
...
![Figure 4.2 – Upcoming arrival details shown on an iPhone (left) and on an Android device (right) ](image/Figure_4.02_B17132.jpg)
...
![Figure 4.3 – App showing a message to indicate a connectivity problem ](image/Figure_4.03_B17132.jpg)

图 4.3 -应用程序显示一条消息，表明连接问题
...
5.  The final step is to add the event handler for the **Send** button:
    ...
    这段代码在服务器上载图像、位置和任何输入文本时显示一个繁忙(活动)指示器。 我们在本章中使用的相同的 WebAPI可以接收这样的上传，并在提交有效数据时返回**“成功”**的消息。 应用程序对此进行检查，并向用户显示适当的消息。
...
![Figure 4.4 – The Quick Report screen running on an iPhone (left) and showing a selected image, and an Android device (right) showing the entry of some dictated text ](image/Figure_4.04_B17132.jpg)
