...
![](assets/17f2aad3-9c5c-4751-a9e4-f7316eb26fd9.png)

接下来，下图显示了一个由几个神经元组成的非常简单的神经网络：

![](assets/205853db-764e-4b05-8b8b-ef2441ae8e7e.png)

这是一个更复杂或更深的网络：

![](assets/798ffc50-fffc-4712-b001-8483da96fd50.png)

# 神经网络训练

既然我们知道了神经网络和神经元是什么，我们就应该讨论它们做什么以及如何做。神经网络是如何学习的？你们这些有孩子的人已经知道这个问题的答案了。如果你想让你的孩子知道什么是猫，你会怎么做？你给他们看猫（图片或真实的）。你想让你的孩子知道什么是狗？让他们看看狗。神经网络在概念上没有什么不同。它有一种**学习规则**的形式，它将修改输入层传入的权重，通过隐藏层进行处理，并通过激活功能将其放入，希望能够识别猫和狗。而且，如果操作正确，猫不会变成狗！
...
![](assets/31b5e961-850a-4227-9931-72415a6f64a7.png)
...
![](assets/a35b5c41-1af3-4ea1-ade3-cb43d49df7a1.png)

如果输入为正或 0，此函数返回 1，否则返回 0。激活功能类似于此的神经元称为**感知器**。这是我们可以开发的最简单的神经网络形式。从视觉上看，它如下所示：

![](assets/f3759083-7ca3-4d33-9172-6d07f3088c51.jpg)
...
![](assets/6833c8ca-216c-493b-8c1f-d9e5433f18cb.png)

一旦输入被*加权*，它们现在需要求和。使用上一个示例，我们将得到：

*6+-4=2*

然后，该总和将通过一个激活函数传递，我们将在后面的章节中详细介绍该函数。这将产生感知器的输出。激活功能最终会告诉感知器触发是否*可以，即激活。*
...
![](assets/8dc93930-0e18-421d-bf82-bcda82021568.png)
...
激活功能是输出之前网络的最后一部分，可被视为输出值的供应商。可以使用多种激活功能，此图仅突出显示其中的一小部分：

![](assets/85e2b8c7-018e-461c-86e0-c1af7841571a.png)

激活函数有线性和非线性两种类型：

*   **线性**：线性函数是位于或接近于直线的函数，如图所示：

![](assets/60046c81-0304-41ac-bc10-5a676f67c74d.png)

*   **非线性**：非线性函数是指不在直线上的函数，如图所示：

![](assets/1062fee4-b892-410a-935b-703e01f01fab.png)

# 视觉激活函数绘图

在处理激活函数时，在使用激活函数之前，您必须直观地了解激活函数的外观。我们将绘制几个激活函数，然后对其进行基准测试，以供您查看：

![](assets/dc5b93de-a4c2-42b4-956a-dd8cf395b292.png)

这就是逻辑斯蒂普近似和 Swish 激活函数在单独绘制时的样子。由于有许多类型的激活函数，下面显示了我们所有的激活函数在一起绘制时的外观：

![](assets/7fea069a-7185-4cfb-89f5-e4e4c66ff234.png)
...
对于这个例子，我们将使用开源软件包**SharpNeat**。它是世界上功能最强大的机器学习平台之一，并附带一个特殊的激活功能绘图仪。您可以在[找到 SharpNeat 的最新版本 https://github.com/colgreen/sharpneat](https://github.com/colgreen/sharpneat) 。对于本例，我们将使用包含的***ActivationFunctionViewer***项目，如图所示：

![](assets/085edbec-9d2a-4552-ace2-052114a39b80.png)
...
*   SharpNeat 激活功能/查看器：SharpNeat（[https://github.com/colgreen/sharpneat](https://github.com/colgreen/sharpneat) ）。*