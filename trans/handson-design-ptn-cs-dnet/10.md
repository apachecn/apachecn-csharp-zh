...
本章包含各种代码示例，以解释反应式编程的概念。代码保持简单，仅用于演示目的。大多数示例都涉及用 C# 编写的.NETCore控制台应用。
...
应用`Map`方法后，我们得到了一个元素的流，序列为 4,6,5。我们已经用一个虚构的例子讨论了反应性是如何工作的。我们创建了一个小型的.NETCore控制台应用，以查看`Merge`、`Filter`和`Map`操作对可观察对象的影响。以下是控制台应用的输出：
...
在上一节中，我们讨论了反应式编程并介绍了它的模型。在本节中，我们将讨论 Microsoft 的反应式编程实现。为了响应.NETCore中的反应式编程，我们提供了各种接口，这些接口提供了在应用中实现反应式编程的方法。
