...
由于数据库处理仅限于专用的应用层，因此在单独的库中定义实体框架核心（`DbContext`是一种良好的做法。因此，我们需要定义一个.NETCore类库项目。正如我们在*第 2 章*、*非功能需求*的*一书用例——了解.NETCore项目*的主要类型中所讨论的，我们有两种不同类型的库项目：**.NET 标准**和**.NET（核心）**。
...
*   **可在任何 Windows 控制台**中使用的工具：这些工具可通过`Microsoft.EntityFrameworkCore.Design`NuGet 软件包获得。所有实体框架核心命令都采用`dotnet ef .....`格式，因为它们包含在`ef`命令行的.NETCore应用中。
