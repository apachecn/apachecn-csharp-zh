...
*   **操作系统**：这是定义将承载 web 应用的操作系统的选项。Windows 和 Linux 均可用于 ASP.NETCore项目。
...
在这里，我们描述了部署 web 应用的最简单方法。在*第 20 章*、*理解 DevOps 原理*和*第 21 章*、*应用 CI 场景的挑战*中，我们将向您介绍Azure DevOps**持续集成/持续交付**（**CI/CD**管道。此管道是另一个Azure 工具集，它自动执行所有必要的步骤，以在生产中获得应用，即构建、测试、在暂存中部署和在生产中部署。
...
在 Windows 中使用.NETCore设计的控制台应用和 web 应用也几乎与 Linux 和 macOS 完全兼容。这意味着您无需再次构建应用，即可在这些平台上运行。此外，非常特定于平台的行为现在具有多平台支持，例如，`System.IO.Ports.SerialPort`类，它从.NET Core 3.0 开始，在 Linux 上。
...
以下脚本可用于在 Linux 中封装命令行.NETCore应用。其想法是该服务的工作原理类似于 Windows 服务。考虑到大多数 Linux 安装都是命令行安装，并且在没有用户登录的情况下运行，这可能很有用：
...
当谈到保护 ASP.NETCore应用时，值得一提的是，该框架有许多功能可以帮助我们解决这一问题。例如，它包括身份验证和授权模式。在 OWASP 备忘单系列中，您将能够阅读许多其他.NET 实践。
...
为您的 web 应用定义身份验证方法。现在有很多认证选项可用，从 ASP.NETCore身份到外部提供商认证方法，如 Facebook 或谷歌。作为软件架构师，您必须考虑应用的目标受众是谁。如果您选择走这条路，那么还值得考虑使用 Azure Active Directory 作为起点。
...
在未启用 HTTPS 协议的情况下部署生产解决方案是完全不可接受的。Azure Web 应用和 ASP.NETCore解决方案不仅可以使用，而且可以强制使用此安全协议。
...
最后，测试将在*第 18 章*、*使用单元测试用例和 TDD 测试代码*、*第 22 章*、*功能测试自动化*中详细讨论。最后，我们有一个 ASP.NETCore应用，我们已经在*中描述了如何使用.NET 5*小节创建一个可伸缩的 web 应用。在这里，我们定义了一个 ASP.NETCore应用，但 Visual Studio 还包含基于 RESTful API 和最重要的单页应用框架的项目模板，如 Angular、React、Vue.js 和基于 WebAssembly 的 Blazor 框架，这些将在*第 16 章**中讨论 Blazor WebAssembly*。其中一些可以通过标准 VisualStudio 安装获得，而另一些则需要安装 SPA 包。
