...
ASP.NETCore是用于实现 web 应用的.NET 框架。ASP.NETCore在前几章中已经做了部分描述，因此本章将主要关注 ASP.NETCore MVC。更具体地说，本章将涵盖以下主题：
...
我们将回顾并进一步详细介绍 ASP.NETCore框架的结构，部分内容已在*第 14 章*、*应用具有.NETCore的面向服务架构*、以及*第 4 章*、*中讨论，以确定最佳的基于云的解决方案*。这里主要关注的是如何基于所谓的**模型-视图-控制器**（**MVC**架构模式）实现基于 web 的表示层。
...
本章讨论了基于 ASP.NETCore框架实现基于 web 的应用表示层的架构。web 应用的表示层基于三种技术：
...
ASP.NETCore基于通用主机的概念，如*第 5 章*中的*使用通用主机*小节所述，*将微服务架构应用于您的企业应用*。在*第 14 章*的*ASP.NETCore简介*小节*中概述了 ASP.NETCore的基本架构，并将面向服务的架构应用于.NETCore*。
...
另一方面，`Configure(IApplicationBuilder app, IWebHostEnvironment env)`启动方法定义了所谓的 ASP.NETCore管道，在*第 14 章*的*ASP.NETCore简介*小节中有简要描述，*应用了具有.NETCore的面向服务架构*，下一小节将对其进行更详细的描述。
...
您可以将所需的所有模块插入一个称为**ASP.NETCore管道**的公共处理框架中，从而将它们组合在一起。

更具体地说，ASP.NETCore请求是通过将上下文对象推送到 ASP.NETCore模块管道来处理的，如下图所示：

<figure class="mediaobject">![](img/B16756_15_01.png)</figure>

图 15.1:ASP.NETCore管道
...
一些模块可以构建一个中间数据结构，然后供管道中的其他模块使用。通常，这种中间数据可以存储在`IDictionary<object, object>`的自定义条目中，该条目包含在`HttpContext`对象的`Items`属性中。但是，有一个预定义属性`User`，其中包含有关当前登录用户的信息。登录用户不是自动计算的，因此必须由身份验证模块计算。*第 14 章*的*ASP.NETCore服务授权*小节*将面向服务的架构应用于.NETCore*中，说明了如何将执行 JWT 令牌身份验证的标准模块添加到 ASP.NETCore管道中。
...
`HttpContext`还允许我们通过依赖项注入引擎的`RequestServices`属性访问依赖项注入引擎。您可以通过调用`.RequestService.GetService(Type t)`方法，或者更好的是，调用建立在依赖项引擎之上的`.GetRequiredService<TService>()`扩展方法，来获取依赖项引擎管理的类型的实例。但是，正如我们将在本章的其余部分中看到的，依赖项注入引擎管理的所有类型通常都会自动注入构造函数，因此这些方法仅在我们构建自定义**中间件**或 ASP.NETCore引擎的其他自定义时使用。
...
通过调用`Startup.cs`文件的`Configure`方法中的`UseMiddleware<T>`方法，在 ASP.NETCore管道中注册模块，如下图：
...
之后，可以通过调用`app.UseMyFunctionality(...)`将整个功能添加到应用中。例如，可以通过调用`app.UseEndpoints(....)`将 ASP.NETCore MVC 功能添加到 ASP.NETCore管道中。
...
下一小节描述 ASP.NETCore框架的另一个重要特性；即，如何处理应用配置数据。

## 加载配置数据并将其与选项框架一起使用

当ASP.NETCore应用启动时，它从`appsettings.json`和`appsettings.[EnvironmentName].json`文件中读取配置信息（如数据库连接字符串），其中`EnvironmentName`是一个字符串值，取决于应用部署的位置。`EnvironmentName`的典型值如下：
...
`[EnvironmentName]`字符串取自`ASPNETCORE_ENVIRONMENT`操作系统环境变量。反过来，`ASPNETCORE_ENVIRONMENT`可以通过两种方式在Visual Studio 部署应用时自动设置：
...
*   您在部署期间必须设置`ASPNETCORE_ENVIRONMENT`的值也可以通过添加以下代码在应用的 Visual Studio ASP.NETCore项目文件（`.csproj`中指定：
...
在 Visual Studio 中开发期间，可以在 ASP.NETCore项目的`Properties\launchSettings.json`文件中指定应用运行时给予`ASPNETCORE_ENVIRONMENT`的值。`launchSettings.json`文件包含几个命名的设置组。这些设置配置从 Visual Studio 运行 web 应用时如何启动。通过使用 Visual Studio 的“运行”按钮旁边的下拉列表选择组名，可以选择应用组的所有设置：
...
在 ASP.NETCore管道定义期间，可以通过`IWebHostEnvironment`接口测试从`ASPNETCORE_ENVIRONMENT`操作系统环境变量加载的当前环境。这是因为将`IWebHostEnvironment`实例作为参数传递给`Startup.cs`文件的`Configure`方法。`IWebHostEnvironment`也可通过 DI 提供给用户代码的其余部分。

`IWebHostEnvironment.IsEnvironment(string environmentName)`检查`ASPNETCORE_ENVIRONMENT`的当前值是否为`environmentName`。对于测试开发（`.IsDevelopment()`、生产（`.IsProduction()`）和阶段（`.IsStaging()`）也有特定的捷径。`IWebHostEnvironment`还包含 ASP.NETCore应用的当前根目录（`.WebRootPath`和为静态文件保留的目录（`.ContentRootPath`），这些静态文件由 web 服务器按原样提供（CSS、JavaScript、图像等）。
...
下一小节描述 ASP.NETCore MVC 应用所需的基本 ASP.NETCore管道模块。
...
`UseCookiePolicy`已在.NET 5 模板中删除，但您仍可以手动添加。它确保通过 ASP.NETCore管道处理 cookie，但前提是用户同意使用 cookie。通过同意 cookie 获得对 cookie 使用的同意；也就是说，只有在请求 cookie 中找到此同意 cookie 时，才会启用 cookie 处理。当用户单击同意按钮时，此 cookie 必须由 JavaScript 创建。包含同意 cookie 名称及其内容的整个字符串可以从`HttpContext.Features`中检索，如以下代码段所示：
...
关于 JWT 认证的更多信息，请参考*第 14 章*中的*ASP.NETCore服务授权*小节*应用具有.NETCore的面向服务架构*。
...
有关授权的更多信息，请参考*第 14 章*中的*ASP.NETCore服务授权*小节*应用具有.NETCore的面向服务架构*。

`UseRouting`和`UseEndpoints`处理所谓的 ASP.NETCore端点。端点是为特定 URL 类服务的处理程序的抽象。这些 URL 被转换成带有模式的`Endpoint`实例。当模式匹配 URL 时，将创建一个`Endpoint`实例，并填充模式名称和从 URL 提取的数据。这是将 URL 部分与模式的命名部分匹配的结果。这可以在以下代码段中看到：
...
如下面的代码片段所示，模式在`UseRouting`中间件中处理，但它们在`UseEndpoints`方法中列出。虽然 URL 模式没有在使用它们的中间件中直接定义似乎有些奇怪，但这主要是为了与以前的 ASP.NETCore版本保持一致。事实上，以前的版本中没有类似于`UseRouting`的方法，而是在管道末端包含了一个独特的中间件。在新版本中，为了与以前的版本保持一致，模式仍然在管道的末尾定义，但现在，`UseEndpoints`只是在应用启动时创建一个包含所有模式的数据结构。然后`UseRouting`中间件对该数据结构进行处理，如下代码所示：
...
`MapControllerRoute`定义与 MVC 引擎相关的模式，将在下一小节中描述。还有其他方法可以定义其他类型的模式。像`.MapHub<MyHub>("/chat")`这样的调用将路径映射到处理**信号器**的集线器，这是在`WebSocket`之上构建的抽象，而`.MapHealthChecks("/health")`将路径映射到返回应用运行状况数据的 ASP.NETCore组件。您还可以直接将模式映射到自定义处理程序，使用`.MapGet`截取 GET 请求，使用`.MapPost`截取 POST 请求。这被称为到代码的**路线。下面的是`MapGet`的一个例子：**
...
在描述了 ASP.NETCore框架的基本结构之后，我们现在可以转向更多特定于 MVC 的特性。下一小节描述控制器并解释它们如何通过 ViewModels 与称为视图的 UI 组件交互。
...
如果在选择 ASP.NETCore项目后出现的窗口中选择 Razor 视图库项目，则还可以将视图预编译到视图库中。
...
在每个web 应用中，多个页面共享相同的结构；例如，相同的主菜单或相同的左或右栏。在 ASP.NETCore中，这种常见结构在称为布局页面/视图的视图中被分解。
...
现在，让我们看看 ASP.NETCore的最新相关功能。

# 最新版本的 ASP.NET Core 有哪些新功能？

ASP.NETCore的主要变化发生在版本 3.0 中：路由引擎是 MVC 引擎的部分，现在也可用于其他处理程序。在以前的版本中，路由和路由是 MVC 处理程序的一部分，并添加了`app.UseMvc(....)`；现在已被`app.UseRouting()`和`UseEndpoints(...)`所取代，它们不仅可以将请求路由到控制器，还可以路由到其他处理程序。
...
整个 ASP.NETCore框架建立在我们在*第 5 章*中分析的设计原则和模式之上，*将微服务架构应用于企业应用*、*第 8 章*、*与 C#-实体框架核心*、*中的数据交互第 11 章*、*设计模式与.NET 5 实现*、*第 12 章*、*了解软件解决方案中的不同领域*、*第 13 章*、*在 C# 9*中实现代码重用。
...
然而，ASP.NETCore还应用了其他模式，这些模式是通用的关注点分离原则的具体实例，这是单一责任原则的推广。详情如下:

*   中间件模块架构（ASP.NETCore管道）
*   从应用代码中分解验证和全球化
*   MVC 模式本身
...
ASP.NETCore管道架构有两个重要优点：

*   根据单一责任原则，对初始请求执行的所有不同操作都被分解成不同的模块。
*   执行这些不同操作的模块不需要相互调用，因为 ASP.NETCore框架会一次性调用每个模块。这样，每个模块的代码就不需要执行与分配给其他模块的职责相关的任何操作。
...
根据关注点分离原则，用于根据用户偏好设置请求区域性的整个逻辑被分解到 ASP.NETCore管道的特定模块中。要配置此模块，作为第一步，我们设置支持的日期/数字区域性，如下例所示：
...
现在我们已经了解了如何在 ASP.NETCore项目中管理全球化，在下一小节中，我们将描述 ASP.NETCore MVC 用于实施*关注点分离*的更重要的模式：MVC 模式本身。
...
在本节中，作为 ASP.NETCore应用的一个示例，我们将实现管理面板，用于管理`WWTravelClub`图书用例的目的地和包。应用将采用*第 12 章*中描述的**领域驱动设计**（**DDD**方法，*理解软件解决方案*中的不同领域。因此，充分理解这一章是阅读本节的基本前提。下面的小节描述了整个应用规范和组织，然后是各个应用部分。
...
*   有一个数据层，包含存储库的实现和描述数据库实体的类。这是一个.NETCore库项目。但是，由于它需要一些接口，如`Microsoft.NET.Sdk.web`中定义的`IServiceCollection`，因此我们不仅要添加对.NET Core SDK 的引用，还要添加对 ASP.NET Core SDK 的引用。可按如下方式进行：
...
`Extensions`文件夹包含`DBExtensions`静态类，该类反过来定义了两个扩展方法，分别添加到应用 DI 引擎和 ASP.NETCore管道中。一旦添加到管道中，这两种方法将连接数据库层和应用层。
...
请确保授权和身份验证模块都已添加到 ASP.NETCore管道中；否则，身份验证/授权引擎将无法工作。
...
最后，让我们将`Microsoft.EntityFrameworkCore.Design`添加到 ASP.NETCore项目中。
...
作为第一步，为了简单起见，让我们通过向 ASP.NETCore管道中添加所遵循的代码，将应用区域性冻结为`en-US`：
...
在本章中，我们详细分析了 ASP.NETCore管道和构成 ASP.NETCore MVC 应用的各种模块，如身份验证/授权、选项框架和路由。然后，我们描述了控制器和视图如何将请求映射到响应 HTML。我们还分析了最新版本中引入的所有改进。
...
1.  能否列出 Visual Studio 在 ASP.NETCore项目中构建的所有中间件模块？
2.  ASP.NETCore管道模块需要从基类继承还是实现一些接口？
